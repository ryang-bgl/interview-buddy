...
export default function FlashcardNoteReviewScreen() {
  ...
  const finalizeSwipe = (direction: 'next' | 'prev') => {
  ...
  const panResponder = useMemo(
    () =>
      PanResponder.create({
        ...
        onPanResponderMove: Animated.event([null, { dx: cardOffset }], {
    ...
    [cardOffset, handleNextCard, handlePreviousCard]
  );
    ...
    return (
      <View>
        ...
        >
          ...
          >
            <Text style={styles.cardLabel}>{showBack ? 'Answer' : 'Prompt'}</Text>
            <Text
              style={[
                styles.cardText,
                !showBack && styles.promptText,
              ]}
            >
              {showBack
            ...
            {currentCard?.tags?.length ? (
              <View style={styles.cardTagRow}>
                {currentCard.tags.map(tag => (
                  <View key={`${currentCard.id}-${tag}`} style={styles.cardTagPill}>
        ...
        {noteComplete ? (
          ...
          <TouchableOpacity style={styles.nextButton} onPress={handleNextCard}>
            <Text style={styles.nextButtonText}>{isLastCard ? 'Finish review' : 'Next card'}</Text>
          </TouchableOpacity>
        )}
      </View>
    );
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        ...
        {note ? (
          <View>
            ...
            <TouchableOpacity style={styles.secondaryLink} onPress={toggleListMembership}>
              <Text style={styles.secondaryLinkText}>
                {isInReviewList ? 'Remove from review list' : 'Add to review list'}
              </Text>
            </TouchableOpacity>
          </View>
        ) : null}
