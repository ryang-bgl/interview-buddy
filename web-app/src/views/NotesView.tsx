      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Every snippet from Text → Review Cards shows up here for editing.
          </p>
        </div>
        <Input
          placeholder="Search notes"
          value={notebookStore.noteSearchQuery}
          onChange={(event) => notebookStore.setNoteSearchQuery(event.target.value)}
          className="lg:w-72"
        />
      </div>
        ...
        {filteredNotes.map((note) => (
          <Card key={note.noteId} className="border-border/70">
            <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <CardTitle className="text-lg">{note.topic ?? note.summary ?? note.url}</CardTitle>
